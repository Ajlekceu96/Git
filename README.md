...
1. Нажать на кнопку [[Create a new repository]](https://github.com/new).
2. Ввести название и описание репозитория.
3. Выбрать видимость репозитория **[Public]** или **[Private]**.
4. Нажать на кнопку **[Create repository]**.
5. Открываем в **терминале** папку с вашим **проект**
...
6. После чего в **терминале** вашего **нового** проекта по очередно вводите команды:
```Git
echo "# Git" >> README.md
git init
git add README.md
...
```
6. Если проект у вас **уже был** и был подключен **контроль версий**, то используйте следующие команды:
```Git
git remote add origin https://github.com/Ваш_никнейм/Название_репозитория.git
git branch -M main
git push -u origin main
